...
(async () => {
  ...
  try {
    var eyesConfig = {
      ...
      batchName: "ACME Bank", //config.batchName,
      batchId: "acme bank" + startTime,
...
*/
    ...
    for (i = 1; i <= urls.length; i++) {
      try {
        await eyes.open(driver, config.appName, "ACME Bank");

        await driver.get("http://localhost:8080//index.html");

        await eyes.check("Login", Target.window().fully());

        await driver.findElement(By.css("a#log-in")).click();

        await eyes.check("Bank", Target.window().fully());
